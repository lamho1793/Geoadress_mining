...
def write_to_queue(q):
    ...
    for index, row in main_data.iterrows():
        ...
        row_number = row[0]
        user_id = row[0]
        addr_dtl = ' '.join(row[1:].astype('str').tolist()).replace('nan ','').replace(' nan','')
...
def read_from_queue_and_process(q, n):
    with open(r'E:\Python_Project\Reagan\Internet_bandwith\Result\address_' + n + '.csv', 'w+', newline='', encoding="utf-8") as csvwitfile:
        headerList = ['Id','latitude', 'longitude', 'score', 'geo_address', 'dc_district', 'region', 'building_name', 'block_no', 'block_descriptor', 'location_name', 'street_name', 'building_no_from', 'phase_name', 'estate_name','Status']
...
def process_address(address: object) -> object:
    ...
    if r.status_code == requests.codes.ok:
        ...
        # process Chinese Address Part
            ...
            return ['NULL','NULL',address]+['NULL']*11+[r.status_code]
        ...
        r.close()
        time.sleep(5)
        return(process_address(address))
    else:
